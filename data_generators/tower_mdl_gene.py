...
class Gene_data(nn.Module):
    def __init__(self, para_min = 40000, min_filter = 26, min_fc = 32, max_filter = 32, max_fc = 256,min_num_brick = 3, max_num_brick=6,max_para_times = 50, default_init_w_algor = True,flag = False):
        super().__init__()
        '''
        ...
        self.flag: if True, all the activity function would be the same as the first random chosen one.
        self.fixed_activity: when flag is True, save the activity type here. default = 'relu'
        '''
    ...
    def create_brick(self):
        ...
        if self.flag == True:
            self.fixed_activity = random.choice(self.randomchoice_act)
            brick.append(random.choice(self.randomchoice_LT))
            brick.append(self.fixed_activity)
            brick.append(random.choice(self.radomchoice_Tower))
        return brick
